"""
EQ scoring algorithm for MyWay Career Assessment System.
"""
from typing import List, Dict, Any
from ..models.question_bank import QuestionBank


class EQScoringAlgorithm:
    """Algorithm for calculating EQ scores."""
    
    FACETS = ['eq_empathy', 'eq_social', 'eq_self_awareness', 'eq_self_regulation']
    
    @staticmethod
    def calculate_eq_score(answers: List[Dict[str, Any]], questions: List[QuestionBank]) -> Dict[str, float]:
        """
        Calculate EQ score using Likert scale normalization.
        ...
        facet_scores = {}
        for facet in EQScoringAlgorithm.FACETS:
            facet_key = facet.replace('eq_', '')
            if facet_key in facet_questions:
                facet_scores[facet] = EQScoringAlgorithm._calculate_facet_score(
    ...
    def get_facet_display_names() -> Dict[str, str]:
        """Get display names for EQ facets."""
        return {
            'eq_empathy': 'Empathy',
    ...
    def validate_answers(answers: List[Dict[str, Any]], questions: List[QuestionBank]) -> bool:
        ...
        required_question_ids = {str(q.id) for q in questions if q.category == 'EQ'}
